...
ServerGameState::ServerGameState(NetworkServer* server)
    ...
    EventBus::instance().subscribe<ClientConnectedEvent>([this](const ClientConnectedEvent& e) {
        onClientConnected(e);
    });

    EventBus::instance().subscribe<ClientDisconnectedEvent>([this](const ClientDisconnectedEvent& e) {
...
void ServerGameState::onClientConnected(const ClientConnectedEvent& e) {
    ...
    Logger::info("Player " + std::to_string(playerId) + " joined (color: " +
                std::to_string(player.r) + "," + std::to_string(player.g) + "," + std::to_string(player.b) + ")");
...
void ServerGameState::onNetworkPacketReceived(const NetworkPacketReceivedEvent& e) {
    if (e.size == 0) return;
...
void ServerGameState::processClientInput(ENetPeer* peer, const uint8_t* data, size_t size) {
    ...
    if (input.inputSequence <= player.lastInputSequence) {
        Logger::info("Received old input sequence from player " + std::to_string(playerId) + ", ignoring");
        return;
    }
    player.lastInputSequence = input.inputSequence;

    // Apply input to player
    applyInput(player, input.moveLeft, input.moveRight, input.moveUp, input.moveDown, 16.67f);
}
