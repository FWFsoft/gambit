...
ClientPrediction::ClientPrediction(NetworkClient* client, uint32_t localPlayerId)
    ...
    EventBus::instance().subscribe<LocalInputEvent>([this](const LocalInputEvent& e) {
        onLocalInput(e);
    });
...
void ClientPrediction::onNetworkPacketReceived(const NetworkPacketReceivedEvent& e) {
    if (e.size == 0) return;
...
void ClientPrediction::reconcile(const StateUpdatePacket& stateUpdate) {
    ...
    if (error > 50.0f) {
        Logger::info("Large prediction error: " + std::to_string(error) + " pixels");
    }
    ...
    for (const auto& input : inputsToReplay) {
        applyInput(localPlayer, input.moveLeft, input.moveRight,
                   input.moveUp, input.moveDown, 16.67f);
    }

    // Remove old inputs from history (server has processed these)
    while (!inputHistory.empty() && inputHistory.front().inputSequence <= stateUpdate.serverTick) {
        inputHistory.pop_front();
    }
}
