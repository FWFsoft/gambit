...
LocalInputEvent publishKeyAndUpdate(InputSystem& inputSystem, int sdlKey, bool isKeyDown) {
...
void publishStateUpdate(uint32_t serverTick, const std::vector<PlayerState>& players) {
    StateUpdatePacket packet;
    packet.serverTick = serverTick;
    packet.players = players;
    auto serialized = serialize(packet);
    EventBus::instance().publish(NetworkPacketReceivedEvent{nullptr, serialized.data(), serialized.size()});
}
...
TEST(StateUpdateSerialization) {
    ...
    auto serialized = serialize(original);
    assert(serialized.size() == 7 + (2 * 27));  // 61 bytes for 2 players (1+4+2+54)
...
TEST(EventBus_MultipleSubscribers) {
    ...
    EventBus::instance().subscribe<UpdateEvent>([&](const UpdateEvent&) { call1++; });
    EventBus::instance().subscribe<UpdateEvent>([&](const UpdateEvent&) { call2++; });
    EventBus::instance().subscribe<UpdateEvent>([&](const UpdateEvent&) { call3++; });
...
TEST(EventBus_MultipleEventTypes) {
    ...
    EventBus::instance().subscribe<UpdateEvent>([&](const UpdateEvent&) { updateCalls++; });
    EventBus::instance().subscribe<RenderEvent>([&](const RenderEvent&) { renderCalls++; });
...
TEST(EventBus_Clear) {
    ...
    EventBus::instance().subscribe<UpdateEvent>([&](const UpdateEvent&) { callCount++; });
    EventBus::instance().publish(UpdateEvent{16.67f, 1});
    assert(callCount == 1);
...
TEST(InputSystem_KeyBindings) {
    ...
    for (const auto& keyTest : keys) {
        ...
        // Test key down
        LocalInputEvent downEvent = publishKeyAndUpdate(inputSystem, keyTest.sdlKey, true);
        ...
        LocalInputEvent upEvent = publishKeyAndUpdate(inputSystem2, keyTest.sdlKey, false);
...
TEST(RemotePlayerInterpolation_PlayerLifecycle) {
    ...
    EventBus::instance().publish(NetworkPacketReceivedEvent{nullptr, leftSerialized.data(), leftSerialized.size()});
...
TEST(RemotePlayerInterpolation_MultipleRemotePlayers) {
    ...
    for (uint32_t id = 2; id <= 4; id++) {
        PlayerState ps{id, float(id * 100), float(id * 100), 0.0f, 0.0f, 100.0f,
                      static_cast<uint8_t>(id * 50), 0, 0};
        states.push_back(ps);
    }
